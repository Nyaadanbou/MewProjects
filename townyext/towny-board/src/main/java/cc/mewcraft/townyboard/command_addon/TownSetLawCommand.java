...
public class TownSetLawCommand extends LawCommand implements CommandExecutor {
    ...
    @Override public boolean onCommand(final @NotNull CommandSender sender, final @NotNull Command command, final @NotNull String label, final @NotNull String[] args) {
        ...
        if (!TownyUniverse.getInstance().getPermissionSource().testPermission(player, "townyboard.command.town.set.law")) {
        ...
        TownyMessaging.sendPrefixedTownMessage(town, plugin.getLang().of("msg_town_board_updated").plain());

        @Subst("minecraft:block.anvil.use")
        String sound = Objects.requireNonNull(plugin.getConfig().getString("sounds.town.sound"));
        float volume = (float) plugin.getConfig().getDouble("sounds.town.volume", 1f);
        float pitch = (float) plugin.getConfig().getDouble("sounds.town.pitch", 1f);
        town.playSound(Sound.sound(Key.key(sound), Sound.Source.MASTER, volume, pitch));
